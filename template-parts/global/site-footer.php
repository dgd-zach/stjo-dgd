		...
		<div class="site-footer__grid">
			...
			<div class="site-footer__col site-footer__newsletter">
				<?php if ( stjo_config_get( 'footer.newsletter.heading' ) ) : ?>
					...
					// NEXTURL as a hidden input (Zach's call; LO reads it from the
					// POST body the same as from the action's query string).
					...
					<?php elseif ( isset( $_GET['newsletter'] ) && 'thanks' === $_GET['newsletter'] ) : // no-JS fallback renders what the AJAX path shows inline. phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
						<p class="site-footer__form-thanks has-yellow-color" role="status"><?php esc_html_e( 'Thank you for subscribing!', 'stjo' ); ?></p>
					<?php endif; ?>
					<form class="site-footer__form" action="<?php echo esc_url( $stjo_nl_relay ? admin_url( 'admin-post.php' ) : stjo_config_get( 'footer.newsletter.action', '#' ) ); ?>" method="POST"
						...
						<?php if ( $stjo_nl_survey ) : ?>
							...
							<?php if ( $stjo_nl_next ) : ?>
							<input type="hidden" name="NEXTURL" value="<?php echo esc_url( home_url( $stjo_nl_next ) ); ?>">
						<?php endif; ?>
